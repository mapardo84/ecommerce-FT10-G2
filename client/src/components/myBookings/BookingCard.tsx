...
const BookingCard = (userData: IUserData) => {
  ...
  return (
    <div className="booking_Container">
      {!bookingStatus && <span className="booking_Cancelled">CANCELLED</span>}
      <IconContext.Provider
        value={{ size: "19px", style: { verticalAlign: "top" } }}
      >
        ...
        <div className="booking_Dates">
          <div>
            <BiCalendarCheck />
            <b style={{ fontWeight: 600 }}> Check In:</b> {checkin}
          </div>
          <div>
            <BiCalendarX />
            <b style={{ fontWeight: 600 }}>Check Out:</b> {checkout}
          </div>
        </div>
        <div className="booking_Price">TOTAL: ${totalPrice}</div>
        <div className="booking_More">
          {actual && bookingStatus ? (
            <Button onClick={showModalCancel} type="primary">
              CANCEL
            </Button>
          ) : (
            <Button></Button>
      ...
      <Modal
        footer={[
          ...
          <Button type="primary" onClick={bookingCancel}>
            CONFIRM
          </Button>,
        ]}
        className="booking_Modal"
      ...
      >
        ...
        {moneyBack ? (
          <div>
            Are you sure you want to cancel this booking? The payment done will
            be added to your postive balance, this will be used in your next
            booking. This action cannot be undone.{" "}
          </div>
        ) : (
          <div>
            Are you sure you want to cancel this booking? We will not refund any
            payment done. You are elegible to recieve the payment as positive
            balance if you cancel the reservation more than 7 days before the
            checkin date.
          </div>
        )}
      </Modal>
